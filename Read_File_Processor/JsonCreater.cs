...
namespace Read_File_Processor
{
    public class JsonCreater
    {
        ...
        public string getDetails_FreshCase(Dictionary<string, string> jsonparameters, string MessageId, string ServiceId = "FreshCase")
        {
            string header = getHeader("CSPi Touchless App", Guid.NewGuid().ToString(), Guid.NewGuid().ToString(), "query", "FreshCase", "Download Data from Customer Portal", Guid.NewGuid().ToString(), MessageId, "1.0.0", "1", "no", "jwt.token");
            string status = getStatus("value", "Desc", "details");
            string BGV_STATUS = string.Empty;

            string data = "\"data\":[{";
            data = data + "\"taskName\":\"capgeminiDownloadCaseData\",";
            data = data + "\"taskId\":\"" + Guid.NewGuid() + "\",";
            data = data + "\"taskSerialNo\":1,";
            data = data + "\"taskSpecs\":{\"downloadData\":{";
            foreach (var item in jsonparameters)
            {
                data = data + "\"" + item.Key + "\":\"" + item.Value.Replace("\"", "").Trim() + "\",";
            }
            data = data.TrimEnd(',');
            data = data + "}}";
            data = data + "}]";
        ...
        {
            string data = "\"data\":[{";
            data = data + "\"taskName\":\"capgeminiDownloadData\",";
            data = data + "\"taskId\":\"" + Guid.NewGuid() + "\",";
            data = data + "\"taskSerialNo\":1,";
            data = data + "\"taskSpecs\":{\"downloadData\":{\"customerName\":\"Capgemini Technologies Services India Limited\",\"action\":\"download\"}}";
            data = data + "}]";
            return data;
        }
        ...
        {
            string header = getHeader("CSPi Touchless App", Guid.NewGuid().ToString(), Guid.NewGuid().ToString(), "query", "downloaddata", "Download Data from Customer Portal", Guid.NewGuid().ToString(), MessageId, "1.0.0", "1", "no", "jwt.token");
        ...
        public string getDownload_Response(string MessageId, string ServiceID = "ServiceID", string Response_Status = "N", string Details = "Download Excel")
        {
            ...
            string header = getHeader("CSPi Touchless App", Guid.NewGuid().ToString(), Guid.NewGuid().ToString(), "query", "capgeminiDownloadData", "Download Data from Customer Portal", Guid.NewGuid().ToString(), Guid.NewGuid().ToString(), "1.0.0", "1", "no", "jwt.token");
            string status = getStatus("", "", "");
